...
public class broadcast{
	...
	/*@ public normal_behavior
		...
		assignable machine.content, machine.chat, machine.inactive, machine.toread, machine.toreadcon, machine.owner, machine.contentsize;
		ensures guard_broadcast(b_c,b_u,b_ul) &&  machine.get_content().equals(\old((machine.get_content().union(new BSet<Integer>(b_c))))) &&  machine.get_chat().equals(\old((machine.get_chat().union(BRelation.cross(b_ul,new BSet<Integer>(b_u)).union(BRelation.cross(new BSet<Integer>(b_u),b_ul)))))) &&  machine.get_inactive().equals(\old((machine.get_inactive().union((BRelation.cross(b_ul,new BSet<Integer>(b_u)).union(BRelation.cross(new BSet<Integer>(b_u),b_ul))).difference(machine.get_active()))))) &&  machine.get_toread().equals(\old((machine.get_toread().union(BRelation.cross(b_ul,new BSet<Integer>(b_u)).difference(machine.get_active()))))) &&  machine.get_toreadcon().equals(\old((machine.get_toreadcon().override(new BRelation<Integer,BSet<Integer>>(new Pair<Integer,BSet<Integer>>(b_c,BRelation.cross(b_ul,new BSet<Integer>(b_u)).difference(machine.get_active()))))))) &&  machine.get_owner().equals(\old((machine.get_owner().union(new BRelation<Integer,Integer>(new Pair<Integer,Integer>(b_c,b_u)))))) &&  machine.get_contentsize() == \old(new Integer(machine.get_contentsize() + 1)); 
	 also
		requires !guard_broadcast(b_c,b_u,b_ul);
		assignable \nothing;
		ensures true; */
