...
public class forward{
	...
	/*@ public normal_behavior
		...
		ensures guard_forward(f_c,f_u,f_ul) &&  machine.get_content().equals(\old((machine.get_content().union(new BSet<Integer>(f_c))))) &&  machine.get_chat().equals(\old((machine.get_chat().union(BRelation.cross(f_ul,new BSet<Integer>(f_u)))))) &&  machine.get_inactive().equals(\old((machine.get_inactive().union(BRelation.cross(f_ul,new BSet<Integer>(f_u)).difference(machine.get_active()))))) &&  machine.get_toread().equals(\old((machine.get_toread().union(BRelation.cross(f_ul,new BSet<Integer>(f_u)).difference(machine.get_active()))))) &&  machine.get_toreadcon().equals(\old((machine.get_toreadcon().override(new BRelation<Integer,BSet<Integer>>(new Pair<Integer,BSet<Integer>>(f_c,BRelation.cross(f_ul,new BSet<Integer>(f_u)).difference(machine.get_active()))))))) &&  machine.get_owner().equals(\old((machine.get_owner().union(new BRelation<Integer,Integer>(new Pair<Integer,Integer>(f_c,f_u)))))) &&  machine.get_contentsize() == \old(new Integer(machine.get_contentsize() + 1)); 
	 also
		requires !guard_forward(f_c,f_u,f_ul);
		assignable \nothing;
		ensures true; */
