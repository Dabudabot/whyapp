...
public class chatting{
	...
	/*@ public normal_behavior
		requires true;
 		assignable \nothing;
		ensures \result <==> (machine.get_chat().has(new Pair<Integer,Integer>(ch_u1,ch_u2)) && machine.get_active().has(new Pair<Integer,Integer>(ch_u1,ch_u2)) && machine.CONTENT.difference(machine.get_content()).has(ch_c) && !machine.get_muted().has(new Pair<Integer,Integer>(ch_u1,ch_u2)) && !machine.get_muted().has(new Pair<Integer,Integer>(ch_u2,ch_u1)) && machine.get_user().has(ch_u1) && machine.get_user().has(ch_u2) && !ch_u1.equals(ch_u2) && !machine.get_toreadcon().domain().has(ch_c) &&  (\forall Integer ch_i;new Enumerated(new Integer(1),machine.get_contentsize()).has(ch_i) && machine.get_chatcontentseq().domain().has(ch_i) && !machine.get_chatcontentseq().apply(ch_i).domain().has(ch_c)) &&  (\forall Integer ch_u;machine.get_chatcontent().domain().has(ch_u) && !machine.get_chatcontent().apply(ch_u).domain().has(ch_c))); */
	public /*@ pure */ boolean guard_chatting( Integer ch_c, Integer ch_u1, Integer ch_u2) {
		return (machine.get_chat().has(new Pair<Integer,Integer>(ch_u1,ch_u2)) && machine.get_active().has(new Pair<Integer,Integer>(ch_u1,ch_u2)) && machine.CONTENT.difference(machine.get_content()).has(ch_c) && !machine.get_muted().has(new Pair<Integer,Integer>(ch_u1,ch_u2)) && !machine.get_muted().has(new Pair<Integer,Integer>(ch_u2,ch_u1)) && machine.get_user().has(ch_u1) && machine.get_user().has(ch_u2) && !ch_u1.equals(ch_u2) && !machine.get_toreadcon().domain().has(ch_c) && true && true);
	}
	...
	public void run_chatting( Integer ch_c, Integer ch_u1, Integer ch_u2){
		if(guard_chatting(ch_c,ch_u1,ch_u2)) {
			...
			machine.set_chatcontentseq((chatcontentseq_tmp.override(new BRelation<Integer,BRelation<Integer,BRelation<Integer,Integer>>>(new Pair<Integer,BRelation<Integer,BRelation<Integer,Integer>>>(new Integer(contentsize_tmp + 1),new BRelation<Integer,BRelation<Integer,Integer>>(new Pair<Integer,BRelation<Integer,Integer>>(ch_c,(new BRelation<Integer,Integer>(new Pair<Integer,Integer>(ch_u1,ch_u2)).union(new BRelation<Integer,Integer>(new Pair<Integer,Integer>(ch_u2,ch_u1)))))))))));
