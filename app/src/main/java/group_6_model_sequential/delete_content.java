...
public class delete_content{
	...
	/*@ public normal_behavior
		requires true;
 		assignable \nothing;
		ensures \result <==> (machine.get_chat().has(new Pair<Integer,Integer>(dc_u1,dc_u2)) && machine.get_active().has(new Pair<Integer,Integer>(dc_u1,dc_u2)) && machine.get_content().has(dc_c) && machine.get_chatcontent().domain().has(dc_u2) && machine.get_chatcontent().apply(dc_u2).domain().has(dc_c) && new BRelation<Integer,Integer>(new Pair<Integer,Integer>(dc_u1,dc_u2)).isSubset(machine.get_chatcontent().apply(dc_u2).apply(dc_c)) && machine.get_user().has(dc_u1) && machine.get_user().has(dc_u2) && machine.get_toreadcon().domain().has(dc_c) && dc_u2.equals(machine.get_owner().apply(dc_c)) && new Enumerated(new Integer(1),machine.get_contentsize()).has(dc_i) && machine.get_chatcontentseq().domain().has(dc_i) && machine.get_chatcontentseq().apply(dc_i).domain().has(dc_c)); */
	public /*@ pure */ boolean guard_delete_content( Integer dc_c, Integer dc_u1, Integer dc_u2, Integer dc_i) {
		return (machine.get_chat().has(new Pair<Integer,Integer>(dc_u1,dc_u2)) && machine.get_active().has(new Pair<Integer,Integer>(dc_u1,dc_u2)) && machine.get_content().has(dc_c) && machine.get_chatcontent().domain().has(dc_u2) && machine.get_chatcontent().apply(dc_u2).domain().has(dc_c) && new BRelation<Integer,Integer>(new Pair<Integer,Integer>(dc_u1,dc_u2)).isSubset(machine.get_chatcontent().apply(dc_u2).apply(dc_c)) && machine.get_user().has(dc_u1) && machine.get_user().has(dc_u2) && machine.get_toreadcon().domain().has(dc_c) && dc_u2.equals(machine.get_owner().apply(dc_c)) && new Enumerated(new Integer(1),machine.get_contentsize()).has(dc_i) && machine.get_chatcontentseq().domain().has(dc_i) && machine.get_chatcontentseq().apply(dc_i).domain().has(dc_c));
	}
